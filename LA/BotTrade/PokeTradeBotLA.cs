...
namespace SysBot.Pokemon
{
    ...
    public class PokeTradeBotLA : PokeRoutineExecutor8LA, ICountBot
    {
        ...
        private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8LA sav, PokeTradeDetail<PA8> poke, CancellationToken token)
        {
            ...
            poke.SendNotification(this, $"Found Trading Partner:{tradePartner.TrainerName}. Waiting for a Pokémon...");
            ...
            //test
            List<PA8> batchPK9s = (List<PA8>)poke.Context.GetValueOrDefault("batch", new List<PA8> { toSend });
            List<bool> skipAutoOTList = (List<bool>)poke.Context.GetValueOrDefault("skipAutoOTList", new List<bool> { false });
            PA8 received = default!;
            ...
            for (var i = 0; i < batchPK9s.Count; i++)
            {
                ...
                if (batchPK9s.Count > 1) poke.SendNotification(this, $"批量:等待交换第{i + 1}个宝可梦{ShowdownTranslator<PA8>.GameStringsZh.Species[pk9.Species]}");

                var needUseTradePartnerInfo = !skipAutoOTList[i];
                if (Hub.Config.Legality.UseTradePartnerInfo && needUseTradePartnerInfo)
                {
                    await SetBoxPkmWithSwappedIDDetailsPLA(pk9, tradePartner, sav, token);
                }
                ...
                (pk9, update) = await GetEntityToSend(sav, poke, offered, pk9, trainer, token).ConfigureAwait(false);
                ...
                if (batchPK9s.Count > 1) poke.SendNotification(this, $"批量:第{i + 1}个宝可梦{ShowdownTranslator<PA8>.GameStringsZh.Species[pk9.Species]}交换完成");
